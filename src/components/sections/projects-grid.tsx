...
export function ProjectsGrid() {
  ...
  useEffect(() => {
    async function fetchProjects() {
      try {
        ...
        // Collect all topics from filtered projects, remove duplicates
        const topicsSet = new Set<string>();
        filteredProjects.forEach((project: GitHubRepo) => {
  ...
  const displayedProjects = projects.filter((project) => {
    // Search filter: check if project name includes search term (case-insensitive)
    const matchesSearch = search === '' || project.name.toLowerCase().includes(search.toLowerCase());

    // Tag filter: if no tags selected, show all; otherwise show if project has ANY of the selected tags
    const matchesTags = selectedTopics.length === 0 ||
      project.topics.some(topic => selectedTopics.includes(topic));

    // Project must match both search AND tags
    ...
    <>
      ...
      <div className="mb-6">
        ...
        <div className="flex sm:hidden justify-between items-center mb-2">
          ...
          <button
            className={`px-4 py-2 rounded-full border text-xs font-medium transition-colors ${selectedTopics.length === 0 ? 'bg-background text-foreground' : 'bg-primary text-primary-foreground'} border-border`}
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2 justify-start sm:hidden">
            ...
            {(search || selectedTopics.length > 0) && (
              <Button
                variant="ghost"
                size="sm"
                onClick={() => { setSearch(''); setSelectedTopics([]); }}
        ...
        <div className="hidden sm:flex flex-wrap items-center gap-3 mt-4 justify-start">
          ...
          {(search || selectedTopics.length > 0) && showTags && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { setSearch(''); setSelectedTopics([]); }}
